    ...
    <div className="flex items-center justify-between">
      <div className="flex gap-1.5 items-center *:cursor-pointer">
        <button
          ...
          className="grid place-items-center w-5 h-5 border-[#616161] border rounded-full"
        ...
        >
          {activeSection === "by-month"
            ? monthYear[0].toUpperCase() + monthYear.slice(1).slice(0, -3)
      ...
      <div className="flex items-center gap-2">
        <Select
          ...
          className="h-6.5"
          placeholder={
            VIEW_OPTIONS.find((v) => v.key === activeSection)?.value || "Месяц"
          }
          onSelected={(newSelected) =>
            navigate("/schedule/" + newSelected.key + "/" + activeParamsEnd)
          }
        />
          ...
          {RADIO_OPTIONS.map((opt) => (
            <label key={opt.value}>
              <input
                ...
                value={opt.value}
                checked={activeParamsEnd.slice(0, 4) === opt.value}
                onChange={() =>
                  navigate("/schedule/" + activeSection + "/" + opt.value)
                }
              />
              <span className="w-9 h-6.5 rounded-full border border-[#E0E0E0] flex items-center justify-center peer-checked:bg-[#EDE8FE] cursor-pointer">
                <svg
                  width={opt.width}
                  height={opt.height}
                  fill="none"
