...
export function CategoryBar({
  ...
  return (
    <div className="flex text-[16px] items-center justify-between border-b border-t border-gray-300">
      <ul className="flex h-full space-x-5">
        {categories.map(({ url, label }) => (
          <li className="h-full" key={url}>
            <Link
              className={`px-3 block h-full py-2.5 cursor-pointer border-b-2 ${activeSection === url || activeSection === label ? "border-[#7816db]" : "border-transparent hover:border-[#7816db]"}`}
              to={url}
            >
              {label}
            </Link>
      ...
      {searchActive && (
        <>
          <form className="flex h-full items-center gap-2.5 px-3 py-1.25 bg-white rounded-[12.5px]">
            ...
            <input
              className="rounded-[12.5px] border px-[14.5px] py-1.25 text-[#616161] hover:bg-gray-200 duration-100"
              type="submit"
          ...
          <PopUpMenu open={searchOptionsOpen}>Hello asd</PopUpMenu>
        </>
      )}
    </div>
  );
