...
import { FilterBar, type FilterPart } from "../../components/page/FilterBar";
...
export function Lesson() {
  ...
  function upsertLesson(newLesson: Lesson) {
    setLessons((prevLessons) => {
      const idx = prevLessons.findIndex((l) => l.lesson === newLesson.lesson);
      if (idx !== -1) {
  ...
  useEffect(() => {
    api.schedules.getTeachers().then((teachers) => {
      // @ts-ignore
      filterData[0].options = teachers.map((teacher) => teacher.employer.name);
      setFilterData([...filterData]);
    });

    api.students.getAllGroups().then((groups) => {
      // @ts-ignore
      filterData[1].options = groups.map((group) => group.name);
      setFilterData([...filterData]);
    });

    api.schedules.getSubjects().then((subjects) => {
      // @ts-ignore
      filterData[2].options = subjects.map((subject) => subject.name);
      setFilterData([...filterData]);
    });
  }, []);
