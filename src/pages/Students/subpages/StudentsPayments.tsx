...
export function StudentsPayments() {
  ...
  return (
    <>
      <FilterBar filterData={filterData} selectedChange={setFiltersSelected} />
      <Table
        data={students
          .filter((student) =>
            filtersSelected.group && filtersSelected.group.length > 0
              ? filtersSelected.group.reduce(
                  (result, group) => result || student.groups.includes(group),
                  false,
                )
              : true,
          )
          .filter((student) =>
            filtersSelected.student && filtersSelected.student.length > 0
          ...
          .map((student) =>
            student.payments.map((payment) => ({
              ...
              group: payment.group,
              status: student.subscriptionActive ? "Активен" : "Не активен",
              date: payment.date,
        ...
        keys={{
          ...
          debt: {
            type: "map",
            str: "Задолженность",
            f: (payment) => (payment.debt > 0 ? payment.debt + " ₽" : "нет"),
          },
        }}
        conditionalClassNames={{
          ...
          debt: (student) => (student.debt > 0 ? "text-[#FF1814]" : ""),
        }}
        rowActions={{
          Открыть: (student) => navigate("/students/profile/" + student.id),
          Изменить: (student) =>
